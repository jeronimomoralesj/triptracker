...
// 1) Read your service‐account JSON from a single‐line env var
const serviceAccount = JSON.parse(process.env.GOOGLE_CREDENTIALS_JSON!)

// 2) Initialize GoogleAuth with in‐memory creds
...
export async function GET(req: NextRequest) {
  ...
  // 2) Detalles only (desayunos/cenas)
  if (type === 'desayuno' || type === 'cena') {
    const range = type === 'desayuno'
      ? 'Sheet1!C27:E30'  // breakfast details + links in E
      : 'Sheet1!H27:J31'  // dinner details + links in J
