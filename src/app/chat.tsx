...
// House-specific colors and styles for chat bubble backgrounds
const getHouseColors = (nombre: string) => {
  const viajero = viajeros.find(v => v.nombre === nombre);
  return viajero?.houseColor || "from-amber-600 to-red-800"; // Default Gryffindor colors
};
...
export default function Chat(): JSX.Element {
  ...
  // Improved outside click handler for dropdown
  useEffect(() => {
  ...
  // Getting current user image for the bubble button
      ...
      <AnimatePresence>
        {showChat ? (
          <motion.div
            ...
            className="fixed inset-0 z-50 flex flex-col bg-[url('/hogwarts-bg.jpg')] bg-cover bg-center"
          >
            {/* Overlay for better text contrast */}
            <div className="absolute inset-0 bg-slate-900/70 backdrop-blur-sm"></div>
            
            {/* Chat header */}
            <header className="relative flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 bg-gradient-to-r from-amber-800 to-red-900 shadow-lg z-10">
              <div className="flex items-center gap-3">
                ...
                />
                <h2 className="text-xl sm:text-2xl font-bold text-amber-300 font-serif">Messenger of Magic</h2>
              </div>
              <motion.button 
            ...
            <main ref={chatContainerRef} className="relative flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 z-0">
              {chatMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  />
                  <p className="text-amber-200 font-serif text-lg">No messages yet. Send the first owl post!</p>
                </div>
              ) : (
                chatMessages.map((msg, index) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`flex items-start gap-3 max-w-3xl ${isCurrentUser ? 'ml-auto' : 'mr-auto'}`}
                    >
                      {!isCurrentUser && (
                      ...
                      <div className={`max-w-xs md:max-w-md lg:max-w-lg`}>
                        <div 
                          className={`relative p-3 rounded-lg shadow-lg ${
                            isCurrentUser 
                              ? `bg-gradient-to-br ${getHouseColors(msg.nombre)} text-white rounded-tr-none` 
                        ...
                        >
                          ...
                          {/* Message tip decoration */}
                          <div 
                            className={`absolute ${isCurrentUser ? '-right-2 top-0' : '-left-2 top-0'} w-4 h-4 
                            ${isCurrentUser 
                              ? `bg-gradient-to-br ${getHouseColors(msg.nombre)}` 
                              : 'bg-slate-800/90'
                            }`} 
                            style={{
            ...
            <form
              ...
              className="relative px-4 py-4 sm:px-6 sm:py-4 border-t border-amber-900/50 bg-gradient-to-b from-slate-900/80 to-slate-800/90 backdrop-blur-sm z-10"
            >
              ...
              <div className="mb-3 relative">
                ...
                >
                  {selectedUser ? (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-xs block text-amber-400/70">{selectedUserDetails?.personaje}</span>
                      </div>
                    </div>
                  ) : (
                  ...
                  <ChevronDown 
                    size={18} 
                    className={`text-amber-400 transition-transform duration-300 ${isSelectOpen ? 'rotate-180' : ''}`}
                  />
                </button>
                
                {/* User Selection Modal/Popup - Centered with better positioning */}
                <AnimatePresence>
                  {isSelectOpen && (
                    <motion.div
                    ...
                    >
                      ...
                      >
                        <div className="p-4 border-b border-amber-700/30 bg-gradient-to-r from-amber-900 to-red-900">
                          <h3 className="text-lg font-bold text-amber-300 font-serif">Choose Your Character</h3>
                        </div>
                  ...
                  {[...Array(6)].map((_, i) => (
                    <div 
                      ...
                      style={{
                        ...
                        top: `${Math.random() * 100}%`,
                        animation: `float-particle ${3 + Math.random() * 2}s infinite ${Math.random() * 2}s`
                      }}
                    />
                  ))}
                </div>
